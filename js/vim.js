...
for(let i=0; i<vim.length; i++){
	...
	scriptName.className += "script-name script-name-vim";
	scripts.className = 'scripts';

	scriptName.insertAdjacentHTML('beforeend', '<h3>' + vim[i].name + '</h3>');

	for(let j=0; j<vim[i].code.length; j++){
		scripts.insertAdjacentHTML('beforeend', vim[i].code[j]);
	}

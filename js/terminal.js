...
for(let i=0; i<terminal.length; i++){
	...
	scriptName.className += "script-name script-name-terminal";
	scripts.className = 'scripts';

	scriptName.insertAdjacentHTML('beforeend', '<h3>' + terminal[i].name + '</h3>');

	for(let j=0; j<terminal[i].code.length; j++){
		scripts.insertAdjacentHTML('beforeend', terminal[i].code[j]);
	}

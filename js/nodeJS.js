...
for(let i=0; i<nodeJS.length; i++){
	...
	scriptName.className += "script-name script-name-nodejs";
	scripts.className = 'scripts';

	scriptName.insertAdjacentHTML('beforeend', '<h3>' + nodeJS[i].name + '</h3>');

	for(let j=0; j<nodeJS[i].code.length; j++){
		scripts.insertAdjacentHTML('beforeend', nodeJS[i].code[j]);
	}

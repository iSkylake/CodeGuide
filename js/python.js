...
for(let i=0; i<python.length; i++){
	...
	scriptName.className += "script-name script-name-python";
	scripts.className = 'scripts';

	scriptName.insertAdjacentHTML('beforeend', '<h3>' + python[i].name + '</h3>');

	for(let j=0; j<python[i].code.length; j++){
		scripts.insertAdjacentHTML('beforeend', python[i].code[j]);
	}
